...
public class TipManager {
    ...
    private static func saveTipList() -> Void {
    ...
    public static func addRandomTip() -> Void {
        if (self.tips.isEmpty) { self.tips = createTipList(); }
        if (self.tips[0].title == "?") {
            let rndIndex = Int.random(in: 0...self.tipsHolder.count - 1);
            UserDefaults.standard.set(rndIndex, forKey: UserDefaultKeys.randomIndex);
            self.tips[0] = self.tipsHolder[rndIndex];
            saveTipList();
            return;
        }
        var index:Int = 1;
        ...
        print("You have found all investing tips");
    }
